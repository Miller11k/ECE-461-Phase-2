...
router.post('/', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, username, token, result, _b, first_name, last_name, tokens, updateResult, updatedUser, error_1;
    return __generator(this, function (_c) {
        switch (_c.label) {
            ...
            case 1:
                _c.trys.push([1, 4, , 5]);
                return [4 /*yield*/, dbConfig_js_1.userDBClient.query("SELECT first_name, last_name, tokens FROM ".concat(dbConfig_js_1.employeeDB, " WHERE username = $1"), [username])];
            case 2:
                result = _c.sent();
                ...
                _b = result.rows[0], first_name = _b.first_name, last_name = _b.last_name, tokens = _b.tokens;
                // Check if the token is valid
                if (!tokens || !tokens.includes(token)) {
                    res.status(401).json({ success: false, message: 'Invalid token' }); // Return authentication error
                    return [2 /*return*/];
                }
                return [4 /*yield*/, dbConfig_js_1.userDBClient.query("UPDATE ".concat(dbConfig_js_1.employeeDB, " \n       SET tokens = NULL \n       WHERE username = $1 \n       RETURNING first_name, last_name, tokens"), [username])];
            case 3:
