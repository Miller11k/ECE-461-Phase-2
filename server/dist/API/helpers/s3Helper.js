 ...
 */
export async function fetchPackage(folder_path, zipName) {
    const s3 = new S3({
        accessKeyId: process.env.AWS_ACCESS_KEY_ID,
        secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY,
        region: process.env.AWS_REGION,
 ...
 */
export async function fetchReadmeMatches(folderPath, regex) {
    const s3 = new S3({
        accessKeyId: process.env.AWS_ACCESS_KEY_ID,
        secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY,
        region: process.env.AWS_REGION,
    ...
    try {
        ...
        const readmeFile = listResponse.Contents.find((file) => file.Key?.toLowerCase().includes("readme"));
 ...
 */
async function clearS3Folder(bucketName, folderKey, s3) {
    try {
        const listResponse = await s3
            .listObjectsV2({
            Bucket: bucketName,
...
export async function handleDuplicateAndUpload(packageName, version, zipFileBase64, bucketName) {
    const s3 = new S3({
        accessKeyId: process.env.AWS_ACCESS_KEY_ID,
        secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY,
        region: process.env.AWS_REGION,
    ...
    try {
        ...
        const headResponse = await s3
            ...
            if (err.code !== "NotFound")
                throw err; // Ignore 'NotFound' errors, as it's expected if the key doesn't exist
        });
        if (headResponse) {
