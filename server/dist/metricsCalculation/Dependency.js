...
export class DependencyPinning extends Metrics {
     ...
     */
    async getDependenciesFromGitHub(owner, repo) {
        try {
     ...
     */
    async calculateDependencyPinning(owner, repo) {
        try {
 ...
 */
export async function DependencyPinningTest() {
    ...
    for (const test of url_to_expected_score) {
        ...
        const threshold = 0.1;
        if (Math.abs(result - test.expectedDependencyPinning) <= threshold &&
        ...
        logger.debug(`Dependency Pinning Response Time: ${dependencyPinning.responseTime.toFixed(6)}s`);
    }
