...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const result = await userDBClient.query(`SELECT tokens FROM ${employeeDB} WHERE username = $1`, [currentUsername]);
        ...
        // Update the username
        const updateResult = await userDBClient.query(`UPDATE ${employeeDB} SET username = $1 WHERE username = $2`, [newUsername, currentUsername]);
        ...
        if (error.code === '23505') { // Check if username is already in use
